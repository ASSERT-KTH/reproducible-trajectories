...
def _recent_trajectories(claude_dir=CLAUDE_DIR, n=N_RECENT):
    ...
    traj_files = list(tz.iter_claude_trajectories(claude_dir))
...
def run():
    ...
    if traj_path is None:
        print(
            "collect-trajectories: no matching trajectory found for staged files",
            file=sys.stderr,
        )
        return
    ...
    if http_status is None:
        print(
            f"collect-trajectories: failed to POST trajectory {traj_path.name}: {response}",
            file=sys.stderr,
        )
    else:
        print(
            f"collect-trajectories: submitted {traj_path.name} "
